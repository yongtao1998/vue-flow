  ...
  <div  v-if="easyFlowVisible" style="height: calc(100% - 20px);width: calc(100% - 20px);padding: 10px;font-size: 12px;">
      ...
      <el-col :span="24">
        <div class="ef-tooltar">
          ...
          <div style="float: right;margin-right: 5px">
            <el-button type="primary" plain  @click="saveData" size="mini">保存</el-button>
          </div>
        </div>
      </el-col>
    </el-row>
    ...
    <div style="display: flex;height: calc(100% - 47px);">
      ...
      <div class="efBg" :class="readOnly ? 'noHasMenu' : 'hasMenu'">
        <div id="efContainer" ref="efContainer" class="container"  style="width: 150%;height: 150%;overflow: hidden;transform-origin: 0 0 0">
      ...
      <div style="border-left: 1px solid #dce3e8;background-color: #FBFBFB">
          <NodeForm ref="nodeForm" :readOnly="readOnly" @setLineLabel="setLineLabel" @closeDialog='closeDialog' :centerDialogVisible='centerDialogVisible' @repaintEverything="repaintEverything" ></NodeForm>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    Bus.$on('formDeleteNode', (val)=>{
      this.data.nodeList = this.data.nodeList.filter(function (node) {
          return node.id !== val.id;
        })
        this.$nextTick(function () {
          this.jsPlumb.removeAllEndpoints(val.id);
        })
  })
  },
  methods:{
