<template>
  <div id="flowChart">
    <div class="operating">
      ...
      <div class="btn-group">
        <div class="btn" @click="setMode('edit')" title="选择模式">
          <i class="iconfont icon-chose"></i>
        </div>
        <div class="btn" @click="setMode('drag')" title="拖拽模式">
...
import { isNull , isNode } from "@antv/util";
export default {
  name: "index",
  components: {},
  mounted() {
  ...
  methods: {
    ...
    initG6() {
       ...
       *点击节点
       */
      self.net.on('node:click', function (ev) {
        // self.isNode = isNode(ev.item);   //是否为Node
       ...
       * 鼠标移入移出事件改变颜色
       */
      self.net.on('node:mouseenter', ev => {
        const item = ev.item;
      ...
      self.net.on('node:mouseleave', ev => {
        const item = ev.item;
        self.net.update(item, {
          color: self.oldColor
        });
